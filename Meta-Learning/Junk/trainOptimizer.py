...
def trainOptimizer():
    ...
    with g.as_default():
        ...
        for t in range(Optimizer_steps):
            ...
            for j in range(len(grad_f)):
                cell = tf.contrib.rnn.MultiRNNCell(
                    [tf.contrib.rnn.BasicLSTMCell(hidden_layer) for _ in range(lstm_layer)])
        ...
        opt = tf.train.GradientDescentOptimizer(lr).minimize(loss,var_list = tvars)
        grads_and_vars = opt.compute_gradients(loss,var_list = tvars)
        grads_and_vars = [(tf.clip_by_global_norm(gv[0],1.0),gv[1]) for gv in grads_and_vars]
        train_op = opt.apply_gradients(zip(grads_and_vars))
