...
\begin{itemize}
	...
	This equation for the $i^{th}$ solution only aggregrates the difference values of objectives with other solutions which are worse than the $i^{th}$ solution. For example objective values of some solutions are given in Table \ref{tab:Drexample}.
	...
	Based on Equation \ref{eqn:KHT_Eqn:4}, the distance based ranking values of Solution A, B, C, D, E and F will be -11, -7, -7, -5, -1, and -8 respectively. Therefore solution A is best in this context followed by solution F, B, C, D and E. The solutions are shown in Figure \ref{fig:drexp}.
	
	\begin{figure}[!htb]
		\centering
		\includegraphics[width=.7\textwidth]{drexp.eps}
		\caption{Solutions}
		\label{fig:drexp}
	\end{figure}
